...
```{r}
#Q?  from ostda parisa??
# This code is run without directory and html file
# I created a file named Hello with html format  specific code for title and body according to the prof codes in class on board, but when I  load this file whoe app does not work  why? 
...
getwd()
#~/Desktop/Hello.html
#~/Documents/educadtion/winter 2023 term 2/visualization 6070/session 3/in_class3_shiny/Hello.html
# This line has problem for me#setwd(" ~/Documents/educadtion/winter 2023 term 2/visualization 6070/session 3/in_class3_shiny/")
getwd()
library(shiny)
# install.packages("wordcloud2")
library(wordcloud2)
...
label = "Word source",
choices = c(
"I Have a Dream speech by Martin Luther King Jr's" = "book", "Names file"="names", "Text of life"="Book2",
...
choices = c("Danish", "Dutch", "English", "Finnish", "French", "German", "Hungarian", "Italian",
...
hr(),
HTML('<p>Report a <a href="https://github.com/AntoineSoetewey/word-cloud/issues">bug</a>
or view the <a href="https://github.com/AntoineSoetewey/word-cloud/blob/master/app.R">code</a>.
Back to <a href="https://www.antoinesoetewey.com/">www.antoinesoetewey.com</a>.</p>')
),
mainPanel(
...
<li>If it is a .csv file, there should be only one column containing all words or sentences (see below
for example files)</li>
<li>Numbers and punctuations will be automatically removed, as well as stop words in the language
of your choice (via the dropdown selector)</li></ul>"),
"Example files:",
...
br(),
br(),
HTML('<p>Report a <a href="https://github.com/AntoineSoetewey/word-cloud/issues">bug</a> or
view the <a href="https://github.com/AntoineSoetewey/word-cloud/blob/master/app.R">code</a>.
Back to <a href="https://www.antoinesoetewey.com/">www.antoinesoetewey.com</a>.</p>'),
br(),
br()
)
...
data <- read.csv("~/Desktop/ihaveadream.csv",
sep = "&",
stringsAsFactors = FALSE
)
data <- data[, 1]
} else if (input$source == "names") {
data <- read.csv("~/Desktop/names.csv", sep = "&", stringsAsFactors = FALSE)
data <- data[, 1]
} else if (input$source == "Book2") {
data <- read.csv("~/Desktop/Book2.csv", sep = "&", stringsAsFactors = FALSE)
data <- data[, 1]
